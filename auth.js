...
async function ativarCodigo(codigo) {
  ...
  if (lic.limite_familias > 0 && lic.familias_ativas >= lic.limite_familias) {
    var msg = lic.tipo === 'individual'
      ...
      : 'Este código atingiu o limite de famílias. Entre em contato.';
  ...
  // Sempre incrementa familias_ativas (independente do limite)
  await sb.from('licencas').update({ familias_ativas: lic.familias_ativas + 1 }).eq('id', lic.id);

  // Se for o primeiro a ativar, vira admin da licença
  if (!lic.licenca_admin_id) {
...
// ══════════════════════════════════════════════════════════
// PAINEL ADMIN (pastor / admin de licença)
// ══════════════════════════════════════════════════════════

// Verifica se o usuário logado é o admin da licença (primeiro a ativar)
async function isLicencaAdmin() {
  var profile = await getProfile();
  if (!profile) return false;
  if (profile.role === ROLES.SUPER) return true;
  if (profile.role === ROLES.ADMIN) return true;
  // Verifica se é o licenca_admin da licença (campo licenca_admin_id)
  if (!profile.licenca_id) return false;
...
async function getAdminData() {
  ...
  if (ids.length > 0) {
    ...
    if (prog.data) {
      ...
      lidas = prog.data.length;
    }
  }
  // Anexa contagem de lidas em cada membro
...
// Remove um membro da licença e libera a vaga
async function removerMembro(membroId) {
  ...
  // Segurança: só admin da licença ou super pode remover
  ...
  // Busca o perfil do membro para pegar o licenca_id
  var sb = getSB();
  ...
  // Remove a licença do perfil do membro
  ...
  // Decrementa o contador da licença
...
// Envia convite por e-mail para entrar na licença
async function convidarMembro(email) {
  ...
  // Salva o convite na tabela de convites
  var sb = getSB();
  var token = Math.random().toString(36).slice(2) + Math.random().toString(36).slice(2);
  var expira = new Date(Date.now() + 7 * 86400000).toISOString(); // 7 dias

  // Verifica se já existe convite pendente para este e-mail nesta licença
  ...
  // Envia e-mail via Supabase Auth (magic link não serve — usamos a tabela e o edge function)
  // Como alternativa simples: retornamos o link para o admin copiar e enviar
  var link = window.location.origin + '/aceitar-convite.html?token=' + token;
  return { ok: true, link: link, email: email };
}

// Aceita um convite pelo token (chamado na página aceitar-convite.html)
async function aceitarConvite(token) {
  ...
  if (new Date(convite.expira_em) < new Date()) return { ok: false, msg: 'Este convite expirou. Peça um novo convite ao administrador.' };

  // Verifica se o usuário está logado
  var session = await getSession();
  if (!session) {
    // Salva o token no sessionStorage para usar após login/cadastro
  ...
  // Verifica se a licença ainda tem vagas
...
async function aceitarPorLicenca(licencaId) {
  ...
  if (lic.limite_familias > 0 && lic.familias_ativas >= lic.limite_familias)
    return { ok: false, msg: 'Todas as vagas desta licença já foram preenchidas.' };
  var session = await getSession();
  if (!session) {
...
async function getSuperStats() {
  ...
  var fam = await sb.from('profiles').select('id', { count: 'exact' }).eq('role', 'user');
  var hj  = await sb.from('progresso').select('id', { count: 'exact' }).gte('updated_at', new Date(Date.now() - 86400000).toISOString());
  return { licencasAtivas: lic.count || 0, familiasAtivas: fam.count || 0, historiasHoje: hj.count || 0 };
}
...
function tipoLabel(tipo) {
  return { demo: '🟡 Demo', comprado: '🟢 Comprado', bencao: '🔵 Bênção ❤️', parceiro: '🟣 Parceiro', individual: '👤 Individual', familiar: '👨‍👩‍👧 Familiar' }[tipo] || tipo;
}
